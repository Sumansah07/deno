...
import { useAdaptiveChat } from '~/lib/hooks/useAdaptiveChat';
...
export const ChatImpl = memo(
  ({ description, initialMessages, storeMessageHistory, importChat, exportChat }: ChatProps) => {
    ...
    const [model, setModel] = useState(() => {
      ...
      return savedModel || 'qwen/qwen3-coder'; // Default to qwen3-coder with automatic fallback
    });
    const [provider, setProvider] = useState(() => {
      const savedProvider = Cookies.get('selectedProvider');
      const openRouterProvider = PROVIDER_LIST.find((p) => p.name === 'OpenRouter');
      return (PROVIDER_LIST.find((p) => p.name === savedProvider) || openRouterProvider || DEFAULT_PROVIDER) as ProviderInfo;
    });
    ...
    const handleError = useCallback(
      (error: any, context: 'chat' | 'template' | 'llmcall' = 'chat') => {
        ...
        // Check if this is a usage limit error (429 or explicit flags)
        if (errorInfo.statusCode === 429 || errorInfo.limitReached || errorInfo.upgradeRequired) {
          setLlmErrorAlert({
            type: 'error',
    ...
    const sendMessage = async (_event: React.UIEvent, messageInput?: string) => {
      ...
      if (!chatStarted) {
        // First message - use append to trigger API call
