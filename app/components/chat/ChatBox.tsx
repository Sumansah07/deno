  ...
  return (
    <div
      className={classNames(
        'relative bg-bolt-elements-background-depth-2 backdrop-blur rounded-lg border border-bolt-elements-borderColor relative w-full max-w-chat mx-auto z-prompt',
        props.chatStarted ? 'p-1' : 'p-3',
        /*
         * {
      ...
      >
        <textarea
          ...
          className={classNames(
            ...
            props.chatStarted ? 'pl-2.5 pt-1.5 pr-10 pb-0.5' : 'pl-4 pt-4 pr-16',
          )}
          onDragEnter={(e) => {
            e.preventDefault();
        ...
        <div className={classNames('flex justify-between items-center text-sm', props.chatStarted ? 'px-1.5 py-1' : 'px-4 pt-2')}>
          ...
          <div className="flex gap-1 items-center">
            <ClientOnly>
              {() => {
                const [kimiEnabled, setKimiEnabled] = useState(() => {
                  const stored = localStorage.getItem('kimiPlanning');
                  return stored !== 'false';
                });
                
                return (
                  <IconButton
                    title={kimiEnabled ? 'Thinking Mode: ON' : 'Thinking Mode: OFF'}
                    className={classNames(
                      'transition-all !p-1 !h-7 !w-7',
                      kimiEnabled && 'ring-2 ring-green-500 bg-green-500/10'
                    )}
                    onClick={() => {
                      const newValue = !kimiEnabled;
                      setKimiEnabled(newValue);
                      localStorage.setItem('kimiPlanning', String(newValue));
                      toast.success(newValue ? 'Thinking Mode Enabled' : 'Thinking Mode Disabled');
                    }}
                  >
                    <div className={classNames('i-ph:brain text-base', kimiEnabled ? 'text-green-500' : 'text-gray-400')} />
                  </IconButton>
                );
              }}
            </ClientOnly>
            {/* Model selector hidden - automatic fallback system enabled */}
            {/* <SimpleModelDropdown
              selectedModel={props.model || 'gemini-2.5-flash'}
