 ...
 * STRICT: Only creates project when explicitly needed with valid name
 */
export async function getOrCreateCurrentProject(
  ...
  if (state.projectId) {
    console.log(`♻️ Using existing project: ${state.projectId}`);
  ...
  // Allow project creation with any name during active session
  // Validation happens at save time in shouldSaveProject()
...
export function setCurrentProject(projectId: string, projectName: string) {
  ...
  // Only update if different project
