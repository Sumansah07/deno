  ...
  try {
    ...
    switch (action) {
      ...
      case 'ai_generation': {
        ...
        // Configurable via env with fallback to defaults
        const tierLimits: Record<string, number> = {
          free: parseInt(process.env.TIER_FREE_SCREENS || '10'),
          pro: parseInt(process.env.TIER_PRO_SCREENS || '100'),
          enterprise: parseInt(process.env.TIER_ENTERPRISE_SCREENS || '1000'),
          enterprise_plus: parseInt(process.env.TIER_ENTERPRISE_PLUS_SCREENS || '2000'),
        };
        
        const maxScreens = tierLimits[tier] || 10;
        ...
        const exceeded = actualScreenCount >= maxScreens;
        
        return {
          allowed: !exceeded,
          reason: exceeded 
            ? `You've reached your limit of ${maxScreens} screens/month. Upgrade to ${tier === 'free' ? 'Indie Accelerator ($5/mo)' : tier === 'pro' ? 'Team Rocket ($15/mo)' : 'Studio Dominator ($30/mo)'} for more screens.`
            : undefined,
        };
      }
