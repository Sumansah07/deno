...
import { getOrCreateCurrentProject, setCurrentProject, clearCurrentProject, getCurrentProjectId } from '~/lib/stores/project';

import { detectProjectCommands, createCommandActionsString } from '~/utils/projectCommands';
import type { ContextAnnotation } from '~/types/context';
import { shouldSaveProject } from '~/lib/utils/chat-session';
 ...
 * STRICT: Only creates project when there's actual content to save
 */
async function getOrCreateSupabaseProjectId(chatId: string, chatName: string): Promise<string> {
  // If chatId is already a UUID, use it directly (existing project)
  ...
  // For non-UUID chatIds, use centralized project store
  // This ensures we reuse the current project instead of creating duplicates
  const currentProjectId = getCurrentProjectId();
  if (currentProjectId) {
    console.log(`♻️ Reusing current project: ${currentProjectId}`);
    return currentProjectId;
  }

  // Only create new project if we have actual content
  console.log(`🆕 Creating new project for chat: ${chatName}`);
        ...
        try {
          ...
          if (response.ok) {
            ...
            if (data.chatHistory && data.chatHistory.length > 0) {
              ...
              // Mark all message IDs as reloaded FIRST to skip action execution
              workbenchStore.setReloadedMessages(data.chatHistory.map((m: Message) => m.id));
              
              // Set messages without modification (keep original annotations)
...
${value.content}
              ...
              // Mark all message IDs as reloaded to skip action execution
  ...
  return {
    ...
    storeMessageHistory: async (
      ...
      const validMessages = messages.filter((m) => 
        !m.annotations?.includes('no-store') && 
        m.content && 
        m.content.trim().length > 0
      );

      if (validMessages.length === 0) {
        console.log('⏭️ Skipping save: No valid messages with content');
        return;
      }

      // Check if we have content to save (files or messages)
      const hasFiles = Object.keys(workbenchStore.files.get()).length > 0;
      if (!hasFiles && validMessages.length === 0) {
        console.log('⏭️ Skipping save: No content to save');
        return;
      }
      ...
      // Sync to Supabase if we have content
      try {
        const projectName = description.get() || firstArtifact?.title || 'My Design';
        const hasFiles = Object.keys(workbenchStore.files.get()).length > 0;
        const hasValidMessages = validMessages.length > 0;

        if (hasFiles || hasValidMessages) {
          const supabaseProjectId = await getOrCreateSupabaseProjectId(finalChatId, projectName);

          // Update URL to use UUID if it changed
          if (supabaseProjectId && supabaseProjectId !== finalChatId) {
            navigateChat(supabaseProjectId);
            setUrlId(supabaseProjectId);
            chatId.set(supabaseProjectId);
          }
          ...
          if (supabaseProjectId) {
            projectSync.queueSync({
              ...
              projectName,
              messages: [...archivedMessages, ...validMessages],
