...
export class DatabaseService {
  ...
  async getUserLimits(userId: string) {
    const { data, error } = await this.client.rpc('get_user_limits', { user_id: userId });
  ...
  /**
   * Check if user can export to Figma
   */
  async canExportToFigma(userId: string): Promise<boolean> {
    const { data, error } = await this.client.rpc('can_export_to_figma', { p_user_id: userId });

    if (error) {
      console.error('Error checking Figma export limit:', error);
      return false;
    }
   ...
   */
  async incrementFigmaExport(userId: string) {
    const { error } = await this.client.rpc('increment_figma_export', { p_user_id: userId });

    if (error) {
      console.error('Error incrementing Figma export:', error);
      throw error;
    }
  }
  ...
  async incrementUsage(
    ...
    // Map event types to usage types
    const usageTypeMap = {
      'ai_generation': 'screen_created',
      'project_created': 'screen_created',
      'screen_created': 'screen_created',
    };

    const { data, error} = await this.client.rpc('increment_usage', {
      p_user_id: userId,
      p_usage_type: usageTypeMap[eventType],
    });
